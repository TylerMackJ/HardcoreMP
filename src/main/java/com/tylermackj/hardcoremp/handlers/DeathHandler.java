		...
		ServerLivingEntityEvents.AFTER_DEATH.register((entity, cause) -> {
			if (entity.isPlayer()) {
				...
				PlayerLookup.world((ServerWorld) entity.getWorld()).forEach(player -> {
					player.sendMessage(Text.literal("Team " + entity.getScoreboardTeam().getDisplayName().getLiteralString() + " has died after " + attemptLengthTS + " on attempt " + entity.getScoreboardTeam().getComponent(ComponentRegisterer.TEAM_DATA).getAttemptCount()));
					if (
						player != entity &&
						 player.isAlive() &&
